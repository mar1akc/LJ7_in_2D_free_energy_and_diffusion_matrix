...
// Compile command:  gcc LJ7in2D_prepare_configurations.c -lm -O3
...
#define NSTEPS 1e7  // the length of the stochastic trajectory that we bin
...
void binning_trajectory_saving_confs(long *bins,double *grid_CV1,double *grid_CV2,
...
void binning_trajectory_saving_confs(long *bins,double *grid_CV1,double *grid_CV2,
	...
	int j,n,k,dim = Natoms*2,j1,j2,ind;
	struct vec2 gauss01;
...
// 		exit(1);
		...
		if( j%100000 == 0 ) printf("Running trajectory: %i steps\n",j);
	}
}
...
int main(void){
	...
	FILE *fpot,*fder1,*fder2,*fder12,*fpar,*fbicubic,*fconf;
...
//  Read the metadynamics data for the Gaussian bumps
...
// Compute coefficient matrices for the bicubic spline	
	...
	double *bin_confs;
	bin_confs = (double *)malloc(Ngrid*dim*sizeof(double));
	
 	CPUbegin=clock(); // start time measurement
 	binning_trajectory_saving_confs(bins,grid_CV1,grid_CV2,Nsteps,Natoms,
